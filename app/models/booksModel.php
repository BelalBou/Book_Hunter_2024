...
function findAllPopulars(PDO $connexion, int $limit = 26): array
{
    $sql = "SELECT b.cover, b.id AS bookID, b.title, CONCAT(a.firstname, ' ', a.lastname) AS author_name, b.resume, AVG(un.note) AS note
            ...
            GROUP BY b.id, b.cover, b.title, a.firstname, a.lastname, b.resume
            ORDER BY note DESC
            LIMIT :limit;";
...
function findOneByBookId(PDO $connexion, int $id): array
{
    $sql = "SELECT 
            ...
            LEFT JOIN users_notations un ON b.id = un.book_id
            WHERE b.id = :book_id
            GROUP BY b.id, b.cover, b.title, b.resume, b.publicated_at, a.firstname, a.lastname, c.name
            ORDER BY note DESC";
...
function findAllByTagId(PDO $connexion, int $id): array
{
    $sql = "SELECT 
            ...
            LEFT JOIN users_notations un ON b.id = un.book_id
            WHERE t.id = :tag_id
            GROUP BY b.id, b.cover, b.title, b.resume, b.publicated_at, a.firstname, a.lastname, t.id, t.name, c.name
            ORDER BY note DESC;";
